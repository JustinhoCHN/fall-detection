...
typedef struct NvDsPersonActionObject {
  gchar *action;    /**< Holds a pointer to the person's action. */
} NvDsPersonActionObject;

/**
 * Holds a fire object's parameters
 */
typedef struct NvDsFireObject {
  gboolean hasFire; 
} NvDsFireObject;
...
typedef struct NvDsPersonInROIObject {
  guint num_roi;    /**< Holds a pointer to the number of person in roi area. */
