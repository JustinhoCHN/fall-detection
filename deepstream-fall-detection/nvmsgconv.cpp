...
generate_object_object (NvDsMsg2pCtx *ctx, NvDsEventMsgMeta *meta)
{
  ...
  switch (meta->objType) {
    ...
    //   jobject = json_object_new ();
    //   if (meta->extMsgSize) {
    //     NvDsPersonInROIObject *dsObj = (NvDsPersonInROIObject *) meta->extMsg;
    //     if (dsObj) {
    //       json_object_set_int_member (jobject, "num_roi", dsObj->num_roi);
    //     }
    //   } else {
    //     json_object_set_int_member (jobject, "num_roi", 0);
    //   }
    //   json_object_set_object_member (objectObj, "person", jobject);
    //   break;
    case NVDS_OBJECT_TYPE_CUSTOM: {
      jobject = json_object_new ();
      if (meta->extMsgSize) {
        NvDsPersonActionObject *dsObj = (NvDsPersonActionObject *) meta->extMsg;
        if (dsObj) {
          json_object_set_string_member (jobject, "action", dsObj->action);
        }
        else {
          json_object_set_string_member (jobject, "action", 0);
        }
        json_object_set_object_member (objectObj, "action", jobject);
      }
      break;
    }
  ...
  for (i = 0; i < size; i++) {
    ...
    if (meta->extMsg && meta->extMsgSize) {
      ...
      switch (meta->objType) {
        ...
        //   NvDsPersonInROIObject *dsObj = (NvDsPersonInROIObject *) meta->extMsg;
        //   if (dsObj) {
        //     ss << "|#|" << dsObj->num_roi;  /*only return the number of roi*/
        //   }
        // }
        //   break;
        case NVDS_OBJECT_TYPE_CUSTOM: {
          NvDsPersonActionObject *dsObj = (NvDsPersonActionObject *) meta->extMsg;
          if (dsObj) {
            ss << "|#|" << to_str(dsObj->action); // if fire detected, send fire information
          }
        }
          break;
        default:
